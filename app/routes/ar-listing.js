...
module.exports = [
  {
    method: 'GET',
    path: '/report-list/ar-listing',
    options: {
      auth: { scope: [holdAdmin, schemeAdmin, dataView] },
      handler: async (request, h) => {
        return h.view('reports-list/ar-listing')
      }
    }
  },
  {
    method: 'GET',
    path: '/report-list/ar-listing/download',
    options: {
      auth: { scope: [holdAdmin, schemeAdmin, dataView] },
      validate: {
        ...
        failAction: async (request, h, err) => {
          ...
          const errors = err.details
            ...
            : []
          const data = { errors }
          return h.view('reports-list/ar-listing', data).code(400).takeover()
        }
      },
      handler: async (request, h) => {
        ...
        try {
          ...
          const selectedData = trackingData.arReportData.map(data => {
            return {
              Filename: data.batch,
          ...
          const baseFilename = config.arListingReportName.slice(0, -4)
          ...
          return h.view('reports-list/ar-listing', { errorMessage: 'Failed to fetch tracking data' })
        }
      }
    }
  }
